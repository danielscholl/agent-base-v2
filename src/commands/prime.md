...
<prime-command>
  ...
  <objective>
    Build a lightweight understanding of the "$1" service/repository structure and conventions.
  ...
  <constraints>
    ...
    <rule>SCOPE: Focus ONLY on the "$1" directory (or workspace root if "all")</rule>
  </constraints>
  ...
  <phase number="1" name="structure-discovery">
    <step name="validate-target">
      ...
      <on-failure>Report that directory "$1" was not found</on-failure>
    </step>

    <step name="file-listing">
      <action>Get file listing for target and summarize structure</action>
      <command>If "all": ls -la to list top-level directories. Otherwise: git ls-files $1/ | head -100</command>
    ...
    <step name="read-readme">
      ...
      <path>$1/README.md (or ./README.md if "all")</path>
  ...
  <phase number="2" name="config-detection">
    <step name="find-config">
      ...
      <search-path>$1/ (or ./ if "all")</search-path>
  ...
  <phase number="3" name="inventory-only">
    <step name="list-commands">
      ...
      <glob>$1/.agent/commands/*.md (or .agent/commands/*.md if "all")</glob>
  ...
  <phase number="4" name="summarize">
    <format>Concise markdown summary for "$1" with:</format>
    <sections>
  ...
  <anti-patterns>
    ...
    <avoid>Exploring directories outside of "$1"</avoid>
  </anti-patterns>
</prime-command>
